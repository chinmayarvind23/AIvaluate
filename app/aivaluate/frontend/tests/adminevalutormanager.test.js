...
describe('Selenium Admin Evaluator Manager Page Test', () => {
  ...
  test('Admin Evaluator Manager page loads and displays content', async () => {
    try {
      ...
      // Wait for redirection to the evaluator manager page
      ...
      // Verify that the Evaluators heading is displayed
      const evaluatorsHeading = await driver.wait(until.elementLocated(By.xpath('//h1[text()="Evaluators"]')), 20000);
      expect(evaluatorsHeading).toBeTruthy();
      console.log('Evaluators heading displayed');
